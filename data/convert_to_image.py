...
if __name__ == "__main__":
    for i in range(0, 50000):
        ...
        for i in range(num_keypoint):
            position_in_image.append(convert_to_image(keypoint_list[i], drone_pos, drone_ori))

        u_vectors = []
        v_vectors = []
        kp_vectors = []
        for i in range(num_keypoint):
            u = 640-int(position_in_image[i][0][0][0])
            v = 480-int(position_in_image[i][0][0][1])
        ...
        # cv2.waitKey(3)
        hms = convertToHM(480, 640, kp_vectors, sigma=2)
